...
// Pass 0
//--------------------------------------------------------------//
string NormalMapping_Pass_0_Model : ModelData = "..\\..\\ShaderFile\\NormalMapping\\Sphere.x";
...
VS_OUTPUT NormalMapping_Pass_0_Vertex_Shader_vs_main( VS_INPUT Input )
{
 ...
 //  float3 worldBinormal = mul(Input.mBinormal, (float3x3)gWorldMatrix );
  // Output.B = normalize(worldBinormal);
   Output.N =  normalize(Input.mNormal);
  Output.B =  normalize(Input.mBinormal);
    Output.T =  normalize(Input.mTangent);
   return Output;
}
...
<
   string ResourceName = "..\\07_NormalMapping\\fieldstone_SM.tga";
>;
...
float4x4 NormalMapping_Pass_0_Pixel_Shader_gWorldMatrix : World;
...
float4 NormalMapping_Pass_0_Pixel_Shader_ps_main(PS_INPUT Input) : COLOR
{
   ...
   tangentNormal = normalize(tangentNormal * 2 - 1);
    ...
    float3x3 TBN = float3x3(  Input.T. x,Input.T.y,Input.T.z,
                               Input.B.x,Input.B.y,Input.B.z,
                               Input.N.x,Input.N.y,Input.N.z);
                               
                               // TBN = transpose(TBN);
                             //float3x3  temp = transpose(NormalMapping_Pass_0_Pixel_Shader_gWorldMatrix);
                               
                               TBN = mul(TBN,NormalMapping_Pass_0_Pixel_Shader_gWorldMatrix);
